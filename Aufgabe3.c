...
void configPorts(void)
{
   ...
   GPIO_PORTM_DIR_R |= ~0x02;														// define input for PM(1)
}
...
void main(void)
{
   ...
   while(1)
   {
		...
		analogSwitch = (GPIO_PORTM_DATA_R & 0x02);												// save new input of SW PM(1)						

		if((adcValue >= 1900) && (adcValue <= 2100))												// defined analog-value-area without moving analog-stick
		{					
			...
			else																								// if keep pressing or releasing analog-stick 
				continue;																					// keep last MATCH-value of TIMER2A
		}   
		else																									// defined analog-value-area with moving analog-stick
		{
			if((adcValue < 1900) && (TIMER2_TAMATCHR_R >= ((16000 - 1) * 0.05)))      	// if analog-stick moves to left (-> 5% of ILV)
			  TIMER2_TAMATCHR_R -= 100;																// reduce MATCH-value of TIMER2A
		   else if((adcValue > 2100) && (TIMER2_TAMATCHR_R <= ((16000 - 1) * 0.95)))  // if analog-stick moves to right (-> 95% of ILV)
			  TIMER2_TAMATCHR_R += 100;																// increase MATCH-value of TIMER2A
		   else																								// if analog-stick was released
			   continue;																					// keep last MATCH-value of TIMER2A
		}

		analogSwitchOld = analogSwitch;																// save old input of SW PM(1)
